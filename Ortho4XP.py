...
if getattr(sys, 'frozen', False):
    Ortho4XP_dir = '..'
    # needed to access https providers with the bin version
...
#####################################################################################################
# XXX: temporarilly setting globals to masks pycharm errors, until I see more clearly what's going on
#####################################################################################################

dir_sep = '/'  # until everything's replaced by os.path.join

# globals from configvars
default_website = 'BI'
default_zl = 16
water_option = 3
sea_texture_params = []
...
overpass_server_list = {"1": "http://api.openstreetmap.fr/oapi/interpreter",
                        "2": "http://overpass-api.de/api/interpreter",
                        "3": "http://overpass.osm.rambler.ru/cgi/interpreter"}
...
shutdown_timer = 60  # Time in seconds to close program / shutdown computer after completion
shutd_msg_interval = 15  # Shutdown message display interval
raster_resolution = 10000  # Image size for the raster of the sniffed land class terrain, not yet used
...
def build_landclass_poly_file(lat0, lon0, build_dir, file_to_sniff, dem_alternative=False):
    ...
    return
    # TODO: find out why this has been disabled, and where to keep it
...
def build_poly_file(lat0, lon0, option, build_dir, airport_website=default_website):
    ...
    if not os.path.exists(build_dir):
        os.makedirs(build_dir)
    str_lat = '{:+.0f}'.format(lat0).zfill(3)
    str_lon = '{:+.0f}'.format(lon0).zfill(4)
    if not os.path.exists(Ortho4XP_dir + dir_sep + "OSM_data" + dir_sep + str_lat + str_lon):
        os.makedirs(Ortho4XP_dir + dir_sep + "OSM_data" + dir_sep + str_lat + str_lon)
    poly_file = build_dir + dir_sep + 'Data' + str_lat + str_lon + '.poly'
    airport_file = build_dir + dir_sep + 'Data' + str_lat + str_lon + '.apt'
    patch_dir = Ortho4XP_dir + dir_sep + 'Patches' + dir_sep + str_lat + str_lon
    dico_nodes = {}
    dico_edges = {}
    ...
    for tag in tags:
        try:
            ...
            pass
        subtags = tag.split('"')
        osm_basename = str_lat + str_lon + '_' + subtags[0][0:-1] + '_' + subtags[1] + '_' + subtags[3]
        osm_filename = os.path.join(Ortho4XP_dir, "OSM_data", str_lat + str_lon,
                                    osm_basename + '.osm')
        osm_errors_filename = os.path.join(Ortho4XP_dir, "OSM_data", str_lat + str_lon,
        ...
        if 'way[' in tag:
            ...
            [dicosmr, dicosmrinner, dicosmrouter, dicosmr_name, dicosmr_icao, dicosmr_ele] = osmrel_to_dicos(
                osm_filename, osm_errors_filename)
    ...
    try:
        ...
        if cover_airports_with_highres:
            filed_some = False
            with open(build_dir + dir_sep + "Data" + str_lat + str_lon + '.apt') as fapt:
                for line in fapt:
                    if 'Airport' in line:  # or 'patch' in line:
                        ...
                        new_entry = [[latmin, lonmin, latmin, lonmax, latmax, lonmax, latmax, lonmin, latmin, lonmin],
                                     str(cover_zl), str(airport_website)]
                        if new_entry not in zone_list:
                            zone_list.append(new_entry)
                            filed_some = True
            if filed_some: print(
                '-> Airports and patched areas have been added to the custom zoomlevel list with ZL' + str(
                    cover_zl) + ' and approx radius of ' + str(cover_extent) + ' km.')
    except:
        pass
...
def osmrel_to_dicos(osm_filename, osm_errors_filename):
    ...
    while not finished_with_file:
        ...
        if '<node id=' in items[0]:
            ...
            if items[1] != 'way':
                efile.write("Relation id=" + str(relid) + " contains member " + items[
                    1] + " which was not treated because it is not a way.\n")
                osm_errors_found = True
                continue
            role = items[5]
            if role == 'inner':
                ...
                efile.write("Relation id=" + str(
                    relid) + " contains a member with role different from inner or outer, it was not treated.\n")
                osm_errors_found = True
                continue
...
def include_patch_data(lat0, lon0, patch_dir, dico_nodes, dico_edges, flat_patch_seeds, sloped_patch_seeds, alt_seeds,
    ...
    for pfilename in os.listdir(patch_dir):
        ...
        try:
            ...
            pfile.readline()  # skip first line
            pfile.readline()  # skip second line
            minlat = 90
            maxlat = -90
            minlon = 180
            maxlon = -180
            ...
            while not finished_with_nodes == True:
                ...
                if '<node' in items:
                    ...
                    nodes_codes[node_id] = strcode([slat, slon])  # slat+'_'+slon
        ...
        for pfilename in os.listdir(patch_dir + dir_sep + pdirname):
            ...
            try:
                ...
                try:
                    ...
                    continue
            bbox_loc = keep_obj8(lat0, lon0, lat_anchor, lon_anchor, alt_anchor, heading_anchor, pfilenamelong,
                                 dico_nodes, dico_edges, alt_seeds)
    ...
    for ny in range(0, Ny + 1):
        for nx in range(0, Nx + 1):
            xcoord = way[0] + ny / Ny * (way[2] - way[0]) + nx / Nx * \
                                                            (way[6] - way[0] + ny / Ny * (
                                                             way[0] + way[4] - way[2] - way[6]))
            ycoord = way[1] + ny / Ny * (way[3] - way[1]) + nx / Nx * \
                                                            (way[7] - way[1] + ny / Ny * (
                                                             way[1] + way[5] - way[3] - way[7]))
            nlat = ycoord + lat0
            nlon = xcoord + lon0
        ...
        if line[0:2] == 'VT':
            ...
            try:
                ...
                for j in range(count // 3):
                    ...
                    if (abs(lata - latb) + abs(lona - lonb)) * (abs(lata - latc) + abs(lona - lonc)) * (
                        abs(latc - latb) + abs(lonc - lonb)) == 0:
                        continue
...
def altitude_and_normals_vec(x, y, alt_dem, ndem):
    ...
    t2 = numpy.array(
        [alt_dem[i][j] for i, j in zip((Nminusny - 1) * (Nminusny >= 1), (nx + 1) * (nx < N) + N * (nx == N))],
        dtype=numpy.float32)
    t3 = numpy.array([alt_dem[i][j] for i, j in zip(Nminusny, (nx + 1) * (nx < N) + N * (nx == N))],
                     dtype=numpy.float32)
...
##############################################################################
# TODO: under construction and not yet used : find out what it's intended for
# # Under construction (?)
...
def build_mesh(lat, lon, build_dir):
    t2 = time.time()
    str_lat = '{:+.0f}'.format(lat).zfill(3)
    str_lon = '{:+.0f}'.format(lon).zfill(4)
    alt_filename = build_dir + dir_sep + 'Data' + str_lat + str_lon + '.alt'
    node_filename = build_dir + dir_sep + 'Data' + str_lat + str_lon + '.1.node'
    ele_filename = build_dir + dir_sep + 'Data' + str_lat + str_lon + '.1.ele'
    poly_file = build_dir + dir_sep + 'Data' + str_lat + str_lon + '.poly'
    apt_filename = build_dir + dir_sep + 'Data' + str_lat + str_lon + '.apt'
    if os.path.isfile(apt_filename) != True or os.path.isfile(poly_file) != True:
        print("You must first build OSM data !")
        return
    mesh_filename = build_dir + dir_sep + 'Data' + str_lat + str_lon + ".mesh"
    obj_filename = build_dir + dir_sep + 'Data' + str_lat + str_lon + ".obj"
...
def obtain_jpeg_row(til_x_left, til_y_top, til_y, zoomlevel, website, big_image):
    ...
    for til_x in range(til_x_left, til_x_left + nbr):
        ...
        if url == 'error':
            small_image = Image.open(Ortho4XP_dir + dir_sep + 'Utils' + dir_sep + 'white.jpg')
            ...
            try:
                ...
                if 'image' in r.headers['Content-Type'] or check_tms_response == False:
                    ...
                    try:
                        ...
                        pass
                    if use_bing_for_non_existent_data:
    ...
    while not successful_download:
        ...
        try:
            ...
            if 'Response [20' in str(r):
                ...
                small_image = Image.open(Ortho4XP_dir + dir_sep + 'Utils' + dir_sep + 'white2048.jpg')
                big_image.paste(small_image, (montx * 2048, monty * 2048))
                successful_download = True
        except requests.exceptions.RequestException as e:
            print(e)
...
def download_textures(strlat, strlon):
    ...
    while not finished:
        if not download_to_do_list:
            ...
            if not os.path.isfile(file_dir + file_name + file_ext):
                if verbose_output:
...
def convert_textures(strlat, strlon, build_dir):
    ...
    while not finished:
        if convert_to_do_list == [] or busy_slots_conv >= max_convert_slots:
            ...
            [file_dir, file_name, file_ext] = filename_from_attributes(strlat, strlon, *texture)
            if not os.path.isfile(build_dir + dir_sep + 'textures' + dir_sep + file_name + '.' + dds_or_png):
                if verbose_output:
...
def build_dsf(lat0, lon0, ortho_list, water_overlay,
    ...
    try:
        ...
        try:
            raster_map_im_mult = Image.open(
                build_dir + dir_sep + 'terrain_map_mult_' + strlat + strlon + '.png').convert("L")
    ...
    for i in range(0, nbr_tri_in):
        ...
        if texture == 'None':
            continue
            # XXX: find out why this has been disabled
        ...
        if tri_type == '0':
            if str(texture) in dico_textures:
                ...
                if ((str(texture) + '_overlay') not in dico_textures) and \
                    ...
                    if not os.path.isfile(build_dir + dir_sep + 'textures' + dir_sep + file_name + '.' + dds_or_png):
                        ...
                        elif os.path.isfile(os.path.join(build_dir, 'textures',
                                                         file_name + '.partial.' + dds_or_png)):
                            file_name += '.partial'
                            if verbose_output:
                                print("   Texture file " + file_name + "." + dds_or_png +
                                      " already present.")
            ...
            if len(texture) < 4:
                texture_overlay_idx = -1
            elif ((tri_type == '1' and not use_masks_for_inland) or (
                    int(texture[2]) < 14 and sea_texture_params != [])):
                if str(texture) + '_overlay' in dico_textures:
                    ...
                    if (str(texture) not in dico_textures) and \
                            ((str(texture) + '_sea_overlay') not in dico_textures):
                        if not os.path.isfile(build_dir + dir_sep + 'textures' + dir_sep +
                                              file_name + '.' + dds_or_png):
                            if 'g2xpl' not in texture[3]:
                                download_to_do_list.append(texture)
                                ...
                                print(
                                    "!!!!!!!!! Missing a required texture, conversion from g2xpl requires new textures !!!!!!!")
                                print(texture)
                                download_to_do_list.append(texture)
                        else:
                            if verbose_output:
            ...
            elif tri_type in ['2', '3'] or use_masks_for_inland:
                if str(texture) + '_sea_overlay' in dico_textures:
                    ...
                    if mask_data != 'None':
                        ...
                        if not os.path.isfile(build_dir + dir_sep + 'textures' + dir_sep + mask_name):
                        ...
                        if (str(texture) not in dico_textures) and \
                            ...
                            if not os.path.isfile(build_dir + dir_sep + 'textures' + dir_sep +
                                ...
                                if 'g2xpl' not in texture[3]:
                                    ...
                                    print(
                                        "!!!!!!!!! Missing a required texture, conversion from g2xpl requires new texture !!!!!!!")
                                    print(texture)
                                    download_to_do_list.append(texture)
                            else:
                                if verbose_output:
        ...
        if tri_type == '0':
            for n in [n1, n3, n2]:  # beware of ordering for orientation !
                if str(n) + '_' + str(texture_idx) in dico_new_pt:
                    ...
                    pools[pool_idx, 7 * pos_in_pool:7 * pos_in_pool + 7] = [pool_nx,
                                                                            pool_ny, 0, round(
                            (1 + normal_map_strength * u) / 2 * 65535), round(
                            (1 + normal_map_strength * v) / 2 * 65535),
            ...
            if not water_overlay:
                continue
            if texture_overlay_idx != -1 and ((tri_type == '1' and not use_masks_for_inland) or int(texture[2]) < 14):
                tri_p = []
                for n in [n1, n3, n2]:  # beware of ordering for orientation !
                ...
                if tri_p[0] == tri_p[2] and tri_p[2] == tri_p[4]:
                    ...
                    # textures[0][pool_idx]+=tri_p
            elif texture_overlay_idx != -1 and (tri_type in ['2', '3'] or use_masks_for_inland):
                tri_p = []
                for n in [n1, n3, n2]:  # beware of ordering for orientation !
                    if str(n) + '_' + str(texture_overlay_idx) in dico_new_pt:
                        [pool_idx, pos_in_pool] = dico_new_pt[str(n) + '_' + str(texture_overlay_idx)]
                        ...
                        pos_in_pool = pools_lengths[pool_idx]
                        dico_new_pt[str(n) + '_' + str(texture_overlay_idx)] = [pool_idx, pos_in_pool]
                        [ms, mt] = st_coord(lat, lon, texture[0], texture[1], texture[2],
                                            'BI')  # Masks are always in epsg:4326!
        ...
        for pool_idx in textures[texture_idx]:
            ...
            if pool_idx != 'cross-pool':
                ...
                for j in range(0, blocks):
                    ...
                    f.write(struct.pack('<B', 255))  # COORDINATE COUNT
                    for k in range(0, 255):
                        f.write(struct.pack('<H', dico_new_pool[
                            textures[texture_idx][pool_idx][510 * j + 2 * k]]))  # POOL IDX
                        f.write(
                            struct.pack('<H', textures[texture_idx][pool_idx][510 * j + 2 * k + 1]))  # POS_IN_POOL IDX
                ...
                if remaining_tri_p != 0:
                    ...
                    for k in range(0, remaining_tri_p):
                        f.write(struct.pack('<H', dico_new_pool[
                            textures[texture_idx][pool_idx][510 * blocks + 2 * k]]))  # POOL IDX
                        f.write(struct.pack('<H', textures[texture_idx][pool_idx][
...
def build_tile(lat, lon, build_dir, mesh_filename, check_for_what_next=True):
    ...
    if clean_unused_dds_and_ter_files:
...
def build_overlay(lat, lon, file_to_sniff):
    ...
    dsfconvertcmd = [dsftool_cmd.strip(), ' -dsf2text '.strip(), file_to_sniff_loc,
                     Ortho4XP_dir + dir_sep + 'tmp' + dir_sep + 'tmp_dsf.txt']
...
def build_masks_legacy(lat, lon, build_dir, mesh_filename_list):
    ...
    if not os.path.exists(masks_dir):
        os.makedirs(masks_dir)
    if not os.path.isfile(masks_dir + dir_sep + 'whole_tile.png') or not keep_old_pre_mask:
        for mesh_filename in mesh_filename_list:
            try:
                f_mesh = open(mesh_filename, "r")
            except:
            ...
            for i in range(0, nbr_tri_in):
                ...
                n3 = int(tmplist[2]) - 1
                tri_type = tmplist[3]
                if (tri_type in ['2', '3']) or (tri_type == '1' and use_masks_for_inland):
                    continue
    ...
    if not use_gimp:
        ...
        if ('dar' in sys.platform) or ('win' not in sys.platform):  # Mac and Linux
            ...
            tmpcmd = '"' + gimp_cmd + '" ' + \
                     ...
                     str(
                         masks_width) + ' \\\"' + Ortho4XP_dir + '\\\\Masks\\\\' + strlat + strlon + '\\\\whole_tile_blured.png\\\")"' + \
                     ' -b "(gimp-quit 0)"'
            tmpf.write(tmpcmd)
            tmpf.close()
    ...
    for nxloc in range(0, nx):
        for nyloc in range(0, ny):
            ...
            if tex_im.getextrema()[1] >= 10:
                tex_im.save(
                    masks_dir + dir_sep + str(til_y_min + nyloc * 16) + '_' + str(til_x_min + nxloc * 16) + '.png')
                tex_im.save(build_dir + dir_sep + "textures" + dir_sep + str(til_y_min + nyloc * 16) + '_' + str(
                    til_x_min + nxloc * 16) + '.png')
    try:
        application.progress_attr.set(100)
        if application.red_flag.get() == 1:
            print("Masks construction process interrupted.")
...
def build_tile_list(tile_list, build_dir_option, read_config, use_existing_mesh, bbmasks, bboverlays):
    ...
    for tile in tile_list:
        ...
        if not os.path.isfile(mesh_filename):
            ...
            try:
                application.earth_window.canvas.delete(
                    application.earth_window.dico_tiles_todo[str(lat) + '_' + str(lon)])
            ...
            if not complex_masks:
                ...
                for closelat in [lat - 1, lat, lat + 1]:
                    for closelon in [lon - 1, lon, lon + 1]:
                        ...
                        closemesh_filename = build_dir + dir_sep + '..' + dir_sep + 'zOrtho4XP_' + strcloselat + \
                                             strcloselon + dir_sep + 'Data' + strcloselat + strcloselon + ".mesh"
...
def point_in_polygon(point, polygon):
    ...
    if change in [1, -1, 0]:
        ...
        if (polygon[2 * len(quadrants) - 2] - point[0]) * (polygon[1] - point[1]) - (
             polygon[2 * len(quadrants) - 1] - point[1]) * (polygon[0] - point[0]) >= 0:
            winding_nbr += 2
        else:
            winding_nbr += -2
...
class Earth_Preview_window(Toplevel):
    ...
    def __init__(self):
        ...
        self.check1 = Checkbutton(self.frame_left, text='Per tile config', anchor=W,
                                  variable=self.ptc, bg="light green", activebackground="light green",
                                  highlightthickness=0)
        self.check2 = Checkbutton(self.frame_left, text='Use existing mesh', anchor=W,
                                  variable=self.uem, bg="light green", activebackground="light green",
                                  highlightthickness=0)
        self.check3 = Checkbutton(self.frame_left, text='Build masks', anchor=W,
                                  variable=self.bbm, bg="light green", activebackground="light green",
                                  highlightthickness=0)
        self.check4 = Checkbutton(self.frame_left, text='Build overlays', anchor=W,
                                  variable=self.bbo, bg="light green", activebackground="light green",
                                  highlightthickness=0)
        ...
        self.shortcuts = Label(self.frame_left,
    ...
    def init_canvas(self):
        self.canvas.config(
            scrollregion=(1, 1, 2 ** self.earthzl * 256 - 1, 2 ** self.earthzl * 256 - 1))  # self.canvas.bbox(ALL))
        try:
            ...
            for dirname in os.listdir(self.working_dir):
                if "zOrtho4XP_" in dirname:
                    ...
                    if os.path.isfile(self.working_dir + dir_sep + dirname + dir_sep + "Earth nav data" + dir_sep + strlatround + strlonround + dir_sep + strlat + strlon + '.dsf'):
                        self.dico_tiles_done[str(lat) + '_' + str(lon)] = self.canvas.create_rectangle(x0, y0, x1, y1,
                                                                                                       fill='blue',
                                                                                                       stipple='gray12')
                        link = Custom_scenery_dir + dir_sep + Custom_scenery_prefix + 'zOrtho4XP_' + strlat + strlon
                        if os.path.isdir(link):
                            if os.path.samefile(os.path.realpath(link), os.path.realpath(
                                                                    self.working_dir + dir_sep + 'zOrtho4XP_' + strlat + strlon)):
                                self.canvas.itemconfig(self.dico_tiles_done[str(lat) + '_' + str(lon)],
                                                       stipple='gray50')
            ...
            for dirname in os.listdir(self.working_dir + dir_sep + 'Earth nav data'):
                for filename in os.listdir(self.working_dir + dir_sep + 'Earth nav data' + dir_sep + dirname):
                    ...
                    self.dico_tiles_done[str(lat) + '_' + str(lon)] = self.canvas.create_rectangle(x0, y0, x1, y1,
                                                                                                   fill='blue',
                                                                                                   stipple='gray12')
            link = Custom_scenery_dir + dir_sep + Custom_scenery_prefix + 'zOrtho4XP_' + os.path.basename(
                self.working_dir)
            if os.path.isdir(link):
    ...
    def toggle_old_stuff(self):
        ...
        for dirname in os.listdir(self.working_dir):
            if "zOrtho4XP_" in dirname:
                ...
                if str(lat) + '_' + str(lon) not in self.dico_tiles_done:
                    self.dico_old_stuff[str(lat) + '_' + str(lon)] = self.canvas.create_rectangle(x0, y0, x1, y1,
                                                                                                  outline='red')
        for dirname in os.listdir(Ortho4XP_dir + dir_sep + "Orthophotos"):
            ...
            [x0, y0] = wgs84_to_pix(lat + 1, lon, self.earthzl)
            [x1, y1] = wgs84_to_pix(lat, lon + 1, self.earthzl)
            if str(lat) + '_' + str(lon) not in self.dico_tiles_done and str(lat) + '_' + str(
                    lon) not in self.dico_old_stuff:
                self.dico_old_stuff[str(lat) + '_' + str(lon)] = self.canvas.create_rectangle(x0, y0, x1, y1,
                                                                                              outline='red')
    ...
    def delete_tile(self):
        if self.working_type == 'legacy':
            ...
            try:
                ...
                strlonround = '{:+.0f}'.format(floor(lon / 10) * 10).zfill(4)
                os.remove(
                    self.working_dir + dir_sep + "Earth nav data" + dir_sep + strlatround + strlonround + dir_sep + strlat + strlon + '.dsf')
            except:
                pass
        self.preview_existing_tiles()
    ...
    def add_tile(self, event):
        ...
        if str(lat) + '_' + str(lon) not in self.dico_tiles_todo:
            ...
            [x0, y0] = wgs84_to_pix(lat + 1, lon, self.earthzl)
            [x1, y1] = wgs84_to_pix(lat, lon + 1, self.earthzl)
            self.dico_tiles_todo[str(lat) + '_' + str(lon)] = self.canvas.create_rectangle(x0, y0, x1, y1, fill='red',
                                                                                           stipple='gray12')
        else:
    ...
    def redraw_canvas(self):
        ...
        if nx0 != self.nx0 or ny0 != self.ny0:
            self.nx0 = nx0
            self.ny0 = ny0
    ...
    def draw_canvas(self, nx0, ny0):
        fileprefix = Ortho4XP_dir + dir_sep + "Previews" + dir_sep + "Earth" + dir_sep + "Earth2_ZL" + str(
            self.earthzl) + "_"
        ...
        self.canv_imgNW = self.canvas.create_image(nx0 * 2 ** self.earthzl * 256 / 8, ny0 * 2 ** self.earthzl * 256 / 8,
                                                   anchor=NW, image=self.photoNW)
        ...
        if nx0 < 2 ** (self.earthzl - 3) - 1:
            ...
            self.canv_imgNE = self.canvas.create_image((nx0 + 1) * 2 ** self.earthzl * 256 / 8,
                                                       ny0 * 2 ** self.earthzl * 256 / 8, anchor=NW, image=self.photoNE)
        ...
        if ny0 < 2 ** (self.earthzl - 3) - 1:
            ...
            self.canv_imgSW = self.canvas.create_image(nx0 * 2 ** self.earthzl * 256 / 8,
                                                       (ny0 + 1) * 2 ** self.earthzl * 256 / 8, anchor=NW,
                                                       image=self.photoSW)
        ...
        if nx0 < 2 ** (self.earthzl - 3) - 1 and ny0 < 2 ** (self.earthzl - 3) - 1:
            ...
            self.canv_imgSE = self.canvas.create_image((nx0 + 1) * 2 ** self.earthzl * 256 / 8,
                                                       (ny0 + 1) * 2 ** self.earthzl * 256 / 8, anchor=NW,
                                                       image=self.photoSE)
    ...
    def compute_size(self):
        ...
        for polygon in self.polygon_list:
            ...
            for j in range(1, len(polyp) // 2):
                ...
                x1 = x2
                y1 = y2
            total_size += abs(area) / 2 * (
            (40000 * cos(pi / 180 * polygon[1][0]) / 2 ** (int(self.zl_combo.get()) + 8)) ** 2) * 2 ** (
            2 * (int(polygon[2]) - 17)) / 1024
...
class Ortho4XP_Graphical(Tk):
    def __init__(self):
        ...
        self.skipdown_check = Checkbutton(self.frame_left, text="Skip downloads",
                                          anchor=W, variable=self.skipd, command=self.set_skip_downloads,
                                          bg="light green",
                                          activebackground="light green", highlightthickness=0)
        self.skipconv_check = Checkbutton(self.frame_left, text="Skip converts",
                                          anchor=W, variable=self.skipc, command=self.set_skip_converts,
                                          bg="light green",
                                          activebackground="light green", highlightthickness=0)
        ...
        self.verbose_check = Checkbutton(self.frame_left, text="Verbose output",
                                         anchor=W, variable=self.verbose, command=self.set_verbose_output,
                                         bg="light green",
                                         activebackground="light green", highlightthickness=0)
        ...
        self.cleanddster_check = Checkbutton(self.frame_left, text="Clean unused dds/ter files",
                                             anchor=W, variable=self.cleanddster, command=self.set_cleanddster,
                                             bg="light green",
                                             activebackground="light green", highlightthickness=0)
        self.complexmasks_check = Checkbutton(self.frame_left, text="Complex masks",
                                              anchor=W, variable=self.complexmasks, command=self.set_complexmasks,
                                              bg="light green",
                                              activebackground="light green", highlightthickness=0)
        self.masksinland_check = Checkbutton(self.frame_left, text="Use masks for inland",
                                             anchor=W, variable=self.masksinland, command=self.set_masksinland,
                                             bg="light green",
                                             activebackground="light green", highlightthickness=0)
    ...
    def build_poly_ifc(self):
        ...
        if self.build_dir_entry.get() == '':
            __BUILD_DIR__ = Ortho4XP_dir + dir_sep + 'Tiles' + dir_sep + 'zOrtho4XP_' + strlat + strlon
        else:
            __BUILD_DIR__ = self.build_dir_entry.get()
            if __BUILD_DIR__[-1] == '/':
                __BUILD_DIR__ = __BUILD_DIR__[:-1] + dir_sep + 'zOrtho4XP_' + strlat + strlon
        ...
        if website != 'None':
            ...
            print("\nStep 1 : Building OSM and patch data for tile " + strlat + strlon + " : ")
            print("--------\n")
            fargs_get_osm = [lat, lon, water_option, __BUILD_DIR__, self.map_choice.get()]
            ...
            fargs_build_poly = [lat, lon, __BUILD_DIR__, file_to_sniff, dem_alternative]
    ...
    def build_mesh_ifc(self):
        global __BUILD_DIR__, curvature_tol, no_small_angles, smallest_angle
        [lat, lon] = self.load_latlon()
        if lat == 'error':
            return
        strlat = '{:+.0f}'.format(lat).zfill(3)
        ...
        print("\nStep 2 : Building mesh for tile " + strlat + strlon + " : ")
        print("--------\n")
        fargs_build_mesh = [lat, lon, __BUILD_DIR__]
    ...
    def build_tile_ifc(self):
        global lat, lon, skip_downloads, skip_converts, verbose_output, \
        ...
        if self.build_dir_entry.get() == '':
            ...
            if __BUILD_DIR__[-1] == '/':
                __BUILD_DIR__ = __BUILD_DIR__[:-1] + dir_sep + 'zOrtho4XP_' + strlat + strlon
        mesh_filename = __BUILD_DIR__ + dir_sep + 'Data' + strlat + strlon + ".mesh"
        website = self.map_choice.get()
        if os.path.isfile(mesh_filename) != True:
            if website != 'None':
                ...
                print("\nTranscoding Tile " + strlat + strlon + " : ")
                print("------------------------\n")
                fargs_re_encode_dsf = [lat, lon, __BUILD_DIR__, file_to_sniff, keep_orig_zuv, dem_alternative, seven_zip]
        ...
        print("\nStep 3 : Building Tile " + strlat + strlon + " : ")
        print("--------\n")
        fargs_build_tile = [lat, lon, __BUILD_DIR__, mesh_filename, True]
        build_tile_thread = threading.Thread(target=build_tile,
                                             args=fargs_build_tile)
        build_tile_thread.start()

    def build_masks_ifc(self):
        global lat, lon, __BUILD_DIR__, water_overlay, masks_width, complex_masks
        [lat, lon] = self.load_latlon()
        if lat == 'error':
            return
        strlat = '{:+.0f}'.format(lat).zfill(3)
        ...
        try:
            ...
            print('_____________________________________________________________' +
                  '____________________________________')
            return
        if not os.path.exists(__BUILD_DIR__ + dir_sep + "textures"):
            os.makedirs(__BUILD_DIR__ + dir_sep + "textures")
        mesh_filename = __BUILD_DIR__ + dir_sep + 'Data' + strlat + strlon + ".mesh"
        if os.path.isfile(mesh_filename) != True:
            print("You must first construct the mesh !")
            return
        ...
        if complex_masks == False:
            ...
            for closelat in [lat - 1, lat, lat + 1]:
                for closelon in [lon - 1, lon, lon + 1]:
                    strcloselat = '{:+.0f}'.format(closelat).zfill(3)
                    strcloselon = '{:+.0f}'.format(closelon).zfill(4)
                    closemesh_filename = __BUILD_DIR__ + dir_sep + '..' + dir_sep + 'zOrtho4XP_' + strcloselat + strcloselon + \
                                         dir_sep + 'Data' + strcloselat + strcloselon + ".mesh"
                    if os.path.isfile(closemesh_filename):
                        mesh_filename_list.append(closemesh_filename)
        fargs_build_masks = [lat, lon, __BUILD_DIR__, mesh_filename_list, maskszl]
    ...
    def read_cfg(self):
        global __BUILD_DIR__, water_option, ratio_water, min_area, curvature_tol, \
        ...
        if self.build_dir_entry.get() == '':
            ...
            if __BUILD_DIR__[-1] == '/':
                __BUILD_DIR__ = __BUILD_DIR__[:-1] + dir_sep + 'zOrtho4XP_' + strlat + strlon
        try:
            exec(open(__BUILD_DIR__ + dir_sep + 'Ortho4XP.cfg').read(), globals())
        except:
...
def read_dsf(lat, lon, build_dir, file_to_sniff, keep_orig_zuv=False, dem_alternative=False):
        ...
        while file.tell() < demsend:
            ...
            if flags & 3 == 0:  # float
                ...
                if flags & 3 == 2:  # unsigned
                    fmt = fmt.upper()
            data = numpy.fromstring(file.read(bpp * width * height), '<' + fmt).reshape(width, height).astype(
                numpy.uint16)
    ...
    while file.tell() < cmdsend:
        ...
        if c == 13:  # Polygon Range
            ...
            tri_list_loc = numpy.array([pool[p][d] for (p, d) in numpy.fromstring(bstring_in, '<H').reshape(-1, 2)])[
                fanindices[l]]
            for j in range(l - 2):
                tri_list_kept[curter + '_' + str(flags)].append(tri_list_loc[3 * j:3 * j + 3])
                # TODO FIX !!!
...
def re_encode_dsf(lat0, lon0, build_dir, file_to_sniff, keep_orig_zuv=False, dem_alternative=False, seven_zip=False):
    ...
    (terrain, tri_list_kept, kept_pools, bPROP, bOBJT, bPOLY, bNETW, bDEMN, bGEOD, bDEMS, bCMDS, bathdem,
     nbath) = read_dsf(lat0, lon0, build_dir, file_to_sniff, keep_orig_zuv, dem_alternative)
    ...
    (pool_nbr, pools_params, pools_planes, pools_lengths, pools, textures) = build_pools(lat0, lon0, tri_list_kept,
                                                                                         terrain, bathdem, nbath)
    t2 = time.time()
    # print(t2-t1)
    dico_new_pool = {}
    ...
    if seven_zip:
        ...
        print("  Gain : ", int(
            (os.path.getsize(file_to_sniff) - os.path.getsize(dsf_filename + '.7z')) / os.path.getsize(
                file_to_sniff) * 100), " %")
        ...
        for tri in tri_list_kept[key]:
            ...
            for i in [0, 1, 2]:
                ...
                if key2 in dico_new_pt:
                    ...
                    # if key[-1]=='2':
                    pools[pool_idx, 7 * pos_in_pool:7 * pos_in_pool + 7] = [pool_nx,
                                                                            pool_ny, 0, 32768, 32768, 0,
                                                                            round(deep_factor * 65535)]
                else:
                    print("Warum ?")
                    continue
                    # XXX: find out why this has been disabled
        ...
        for pool_idx in textures[key]:
            ...
            if pool_idx != 'cross-pool':
                ...
                if remaining_tri_p != 0:
                    ...
                    for k in range(0, remaining_tri_p):
                        f.write(
                            struct.pack('<H', dico_new_pool[textures[key][pool_idx][510 * blocks + 2 * k]]))  # POOL IDX
...
if __name__ == '__main__':
    ...
    if water_option in [2, 3]:
        ...
        water_overlay = False
    if __BUILD_DIR__ == "default":
        __BUILD_DIR__ = Ortho4XP_dir + dir_sep + 'Tiles' + dir_sep + 'zOrtho4XP_' + strlat + strlon
    print("\nStep 1 : Building OSM and patch data for tile " + strlat + strlon + " : ")
    print("--------\n")
    build_poly_file(lat, lon, water_option, __BUILD_DIR__)
    print("\nStep 2 : Building mesh for tile " + strlat + strlon + " : ")
    print("--------\n")
    build_mesh(lat, lon, __BUILD_DIR__)
    print("\nStep 2.5 : Building Masks for Tile " + strlat + strlon + " : ")
    print("----------\n")
    mesh_filename = __BUILD_DIR__ + dir_sep + 'Data' + strlat + strlon + ".mesh"
    if not complex_masks:
        ...
        for closelat in [lat - 1, lat, lat + 1]:
            for closelon in [lon - 1, lon, lon + 1]:
                strcloselat = '{:+.0f}'.format(closelat).zfill(3)
                strcloselon = '{:+.0f}'.format(closelon).zfill(4)
                closemesh_filename = __BUILD_DIR__ + dir_sep + '..' + 'zOrtho4XP_' + strcloselat + strcloselon + \
                                     dir_sep + 'Data' + strcloselat + strcloselon + ".mesh"
                if os.path.isfile(closemesh_filename):
                    mesh_filename_list.append(closemesh_filename)
    __BUILD_DIR__ = Ortho4XP_dir + dir_sep + 'Tiles' + dir_sep + 'zOrtho4XP_' + strlat + strlon
    if not os.path.exists(__BUILD_DIR__ + dir_sep + "textures"):
        os.makedirs(__BUILD_DIR__ + dir_sep + "textures")
    build_masks(lat, lon, __BUILD_DIR__, mesh_filename_list, maskszl)
    ortho_list = zone_list[:]
    if default_website != 'None':
        ortho_list.append([[lat, lon, lat, lon + 1, lat + 1, lon + 1, lat + 1, lon, lat, lon],
                           str(default_zl), str(default_website)])
    print("\nStep 3 : Building Tile " + strlat + strlon + " : ")
    print("--------\n")
    build_tile(lat, lon, __BUILD_DIR__, mesh_filename, False)
    print('\nBon vol !')
